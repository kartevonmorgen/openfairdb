...
impl<'c> PlaceRepo for Connection<'c> {
    fn create_or_update_place(&self, place: Place) -> Result<()> {
        let (_place_id, new_place, tags, custom_links) =
            into_new_place_revision(self.deref(), place)?;
    ...
    fn review_places(
        ...
        let changed_by = if let Some(ref email) = activity.by {
            Some(resolve_user_created_by_email(self.deref(), email.as_ref())?)
        } else {
            None
        };
    ...
    fn get_places(&self, place_ids: &[&str]) -> Result<Vec<(Place, ReviewStatus)>> {
        ...
        let rows = query
            .load::<models::JoinedPlaceRevision>(self.deref())
            .map_err(from_diesel_err)?;
    ...
    fn recently_changed_places(
        ...
        let rows = query
            .load::<models::JoinedPlaceRevisionWithStatusReview>(self.deref())
            .map_err(from_diesel_err)?;
        let mut results = Vec::with_capacity(rows.len());
        for row in rows {
    ...
    fn get_place_history(&self, id: &str, revision: Option<Revision>) -> Result<PlaceHistory> {
        ...
        for row in rows {
            ...
            let rows = schema::place_revision_review::table
                ...
                .load::<models::PlaceReviewedRevision>(self.deref())
                .map_err(from_diesel_err)?;
    ...
    fn load_place_revision(&self, id: &str, rev: Revision) -> Result<(Place, ReviewStatus)> {
        ...
        let query = schema::place_revision::table
            ...
            .filter(dsl::id.eq(id));
        let row = query
            .first::<models::JoinedPlaceRevision>(self.deref())
            .map_err(from_diesel_err)?;
        load_place(self.deref(), row)
    }
}
