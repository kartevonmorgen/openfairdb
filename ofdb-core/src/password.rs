...
impl FromStr for Password {
    ...
    fn from_str(password: &str) -> Result<Self, Self::Err> {
        ...
        use pwhash::error::Error as PwErr;
        let res = Self(pwhash::bcrypt::hash(password).map_err(|e| match e {
            PwErr::InsufficientLength => Error::InsufficientLength,
            PwErr::EncodingError => Error::InvalidChar,
            _ => Error::Other,
        })?);
