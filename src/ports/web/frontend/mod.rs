...
pub fn get_search_users(
    ...
    let email = email.url_decode()?;
    {
        let db = pool.shared()?;
        let admin =
            usecases::clearance::user::authorize_by_email(&*db, account.email(), Role::Admin)?;
...
pub fn get_place_history(db: sqlite::Connections, id: &RawStr, account: Account) -> Result<Markup> {
    ...
    let place_history = {
        ...
        usecases::clearance::user::authorize_by_email(&*db, &account.email(), Role::Scout)?;
...
pub fn get_place_review(db: sqlite::Connections, id: &RawStr, account: Account) -> Result<Markup> {
    ...
    let reviewer_email =
        usecases::clearance::user::authorize_by_email(&*db, &account.email(), Role::Scout)?.email;
...
fn review_place(
    ...
    let reviewer_email = {
        let db = db.shared()?;
        usecases::clearance::user::authorize_by_email(&*db, email, Role::Scout)?.email
    };
